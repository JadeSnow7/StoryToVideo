...
* **客户端框架：** C++ (Qt 6.x) 和 QML。
* **服务端 API 地址：** 通过环境变量 `STORYTOVIDEO_API_BASE_URL` 配置（默认 `http://127.0.0.1:8080`）。
...
| 模块 | 文件 | 职责 |
| :--- | :--- | :--- |
| **视图 (View)** | `qml/main.qml`, `qml/AssetsPage.qml` 等 | 渲染 UI，管理页面跳转，发起业务操作。 |
| **视图模型 (ViewModel)** | `src/ViewModel.h`, `src/ViewModel.cpp` | 管理任务状态，驱动轮询，处理业务逻辑，转换数据格式。 |
| **网络管理 (Manager)** | `src/NetworkManager.h`, `src/NetworkManager.cpp` | 封装所有 API 请求细节，解析 `TaskID`，转发网络信号。 |
...
| 文件 | 角色/功能 | 关键实现逻辑 |
| :--- | :--- | :--- |
| **`qml/main.qml`** | **应用主入口与导航容器** | 定义 `ApplicationWindow`，使用 `StackView` (ID: `pageStack`) 作为核心导航结构。 |
...
### 3.2 📂 `src/NetworkManager.cpp` (API 驱动实现)
...
### 3.3 🧠 `src/ViewModel.cpp` (业务逻辑与状态管理实现)
...
| 文件名 | 类型 | 主要功能 | 核心职责 |
| :--- | :--- | :--- | :--- |
| **`src/main.cpp`** | C++ | 程序入口 | 实例化 `ViewModel`，通过 `setContextProperty` 暴露给 QML. |
| **`src/NetworkManager.h`** | H | 接口定义 | 声明所有 API 函数和用于通知 `ViewModel` 的信号 (`taskCreated`, `taskStatusReceived`). |
| **`src/NetworkManager.cpp`** | C++ | API 实现 | 实现所有 HTTP/JSON 交互，包括 `POST /projects` 和 `GET /tasks/{id}` 的细节. |
| **`src/ViewModel.h`** | H | 逻辑模型定义 | 声明 `Q_INVOKABLE` 接口、槽函数、`QTimer` 和 `m_activeTasks`. |
| **`src/ViewModel.cpp`** | C++ | 业务实现 | 实现任务调度、`QTimer` 轮询、信号连接、数据格式转换和结果分发. |
