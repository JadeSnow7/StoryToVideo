...
## 1. 总体架构
- **三层模型**：PC 客户端（Qt/QML） ←→ 云端服务（Django/DRF + 任务队列） ←→ 本地模型节点（FastAPI，经 FRP 暴露），实现算力与业务解耦。
- **通信模式**：REST + WebSocket，长任务由队列驱动；WS 推送状态，HTTP 轮询兜底，保证弱网可恢复。
- **核心目标**：UI 流畅、任务可追踪、资源可复用，并能快速切换算力节点。

> 【图示占位】整体架构拓扑图（客户端-网关-模型节点）。

## 2. 关键模块
### 客户端（Qt/QML + C++)
- **UI/状态管理**：ViewModel 单向数据流，统一状态枚举 PENDING/RUNNING/SUCCESS/FAILED，避免状态分叉。
- **网络层**：QNetworkAccessManager + WebSocket；失败自动重试 + 指数退避；任务轮询兜底，弱网更稳健。
- **媒体能力**：FFmpeg + OpenGL 播放/合成，支持截图、进度控制与 MP4 导出，播放器与导出共用解码缓存。
- **存储**：SQLite + 文件缓存保存项目、图片、视频；离线可读历史内容，缓存命中优先。

### 服务端（Django/DRF 或 Gin）
- **API 网关**：鉴权、限流、日志；统一错误码；反向代理静态资源并透传 WebSocket。
- **任务队列**：Celery/RQ，支持超时、重试、幂等，写入 Task 表；完成后触发 Webhook/WS 回写。
- **数据存储**：PostgreSQL/MySQL；对象存储 OSS/TOS 保存图片/音频/视频；可选 CDN 加速与跨域访问控制。
...
> 【图示占位】端到端流程泳道图（客户端/网关/Worker/OSS）。
...
> 【图示占位】媒体流水线分阶段示意（下载/转码/拼接/混音/导出）。
